...
export class WidgetManager {
	 ...
	 * @param component 组件配置
	 * @param index 组件在列表中的索引
	 * @param sidebar 组件所在的侧边栏（可选，如果未提供则自动检测）
	 */
	getComponentClass(
		...
		// 自动检测侧边栏位置
		if (!targetSidebar) {
			if (this.config.leftComponents.includes(component)) {
				targetSidebar = "left";
			} else if (this.config.rightComponents.includes(component)) {
				targetSidebar = "right";
			}
		}
		...
		if (this.config.position === "both" && targetSidebar === "right") {
