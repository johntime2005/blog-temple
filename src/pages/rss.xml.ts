...
export async function GET(context: APIContext) {
	...
	for (const { post, html } of processedPosts) {
		// Security Check: Skip private posts
		if (post.data.visibility === "private") {
			continue;
		}

		let finalContent = "";
		// Security Check: Hide content for encrypted posts
		if (post.data.encrypted === true) {
			finalContent = sanitizeXmlContent(
				post.data.description || "This content is protected by password.",
			);
		} else {
			finalContent = sanitizeHtmlForXml(
