...
$: {
	...
	let filteredPosts: Post[] = uniquePosts([...sortedPosts, ...privatePosts]);
	...
	if (groupedPostsArray.length > 1 && collapsedYears.size === 0) {
...
<div class="card-base px-8 py-6">
		...
		<div class="mb-5">
			<div class="flex flex-wrap items-center justify-between gap-x-4 gap-y-1">
				<div class="min-w-0 text-sm text-75">
					<a
						href={primaryFilter.labelKey === I18nKey.categories
							? "/categories/"
							: "/tags/"}
						class="text-50 hover:text-(--primary) transition-colors"
					>
						{i18n(primaryFilter.labelKey)}
					</a>
					...
					{#if secondaryFilters.length > 0}
						<span class="ml-2 text-50">
							· {formatFilterSummary(secondaryFilters)}
						</span>
					{/if}
				</div>
				<div class="shrink-0 text-xs text-50">
					{filteredPostCount}
					{i18n(filteredPostCount === 1 ? I18nKey.postCount : I18nKey.postsCount)}
				</div>
			</div>
		</div>
	{/if}
	...
	{#each groups as group}
		<div data-year={group.year}>
			<button
				type="button"
				class="flex flex-row w-full items-center h-15 cursor-pointer rounded-lg hover:bg-(--btn-plain-bg-hover) transition-colors group/yr"
			...
			>
				...
				<div class="w-[70%] md:w-[80%] transition text-left text-50 flex items-center gap-2 group-hover/yr:text-(--primary)">
					...
					<span
						class="archive-arrow"
						style={collapsedYears.has(group.year)
				...
				{#each group.posts as post}
					...
					>
						<div class="flex flex-row justify-start items-center h-full">
							...
							<div class="w-[70%] md:max-w-[65%] md:w-[65%] text-left font-bold group-hover:translate-x-1 transition-all group-hover:text-(--primary) text-75 pr-8 whitespace-nowrap text-ellipsis overflow-hidden flex items-center gap-2">
								{#if post.data.category}
									<span
										class="shrink-0 inline-block text-xs font-medium px-1.5 py-0.5 rounded bg-[oklch(0.95_0.025_var(--hue))] dark:bg-[oklch(0.25_0.025_var(--hue))] text-(--primary) group-hover:bg-(--primary) group-hover:text-white! transition-colors"
									>
										{post.data.category}
									</span>
								{/if}
