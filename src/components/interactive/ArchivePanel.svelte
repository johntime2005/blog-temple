...
onMount(async () => {
	...
	if (token) {
		try {
			...
			console.error("Failed to fetch private posts for archive", e);
		}
	}
});
...
$: {
	let allPosts = [...sortedPosts, ...privatePosts];
	// Remove duplicates if any (though IDs should be unique usually, private API might return same if visibility logic overlaps, but here private API returns distinct private posts)
	// Actually, `getSortedPosts` on server likely returns public only for the prop.
