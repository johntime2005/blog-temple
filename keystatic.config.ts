...
// 客户端可用的环境变量（通过 Vite define 注入）
const GITHUB_CLIENT_ID = import.meta.env.GITHUB_CLIENT_ID;
const GITHUB_OWNER_USERNAME =
	import.meta.env.GITHUB_OWNER_USERNAME || "johntime2005";
...
export default config({
	storage,
	// clientId 用于前端发起 OAuth 流程，clientSecret 仅在 API route 服务端使用
	...(!import.meta.env.DEV && {
		github: {
	...
	collections: {
		...
		categories: collection({
			...
			schema: {
				...
				syncToPublic: fields.checkbox({
					label: "Sync to Public",
					defaultValue: false,
				}),
				encrypted: fields.checkbox({ label: "Encrypted", defaultValue: false }),
				...
				icon: fields.text({
					label: "Icon (Iconify)",
					defaultValue: "material-symbols:folder",
				}),
		...
		siteConfig: singleton({
			...
			schema: {
				...
				navbar: fields.object(
					{
						...
						widthFull: fields.checkbox({
							label: "Full Width",
							defaultValue: false,
						}),
						followTheme: fields.checkbox({
							label: "Follow Theme",
							defaultValue: false,
						}),
					},
				...
				outdatedThreshold: fields.number({
					label: "Outdated Threshold (Days)",
					defaultValue: 30,
				}),
				sharePoster: fields.checkbox({
					label: "Enable Share Poster",
					defaultValue: true,
				}),
				...
				postListLayout: fields.object(
					{
						...
						allowSwitch: fields.checkbox({
							label: "Allow Switch",
							defaultValue: true,
						}),
						grid: fields.object({
							masonry: fields.checkbox({
								label: "Masonry Grid",
								defaultValue: false,
							}),
