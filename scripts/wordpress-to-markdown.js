...
function convertHTMLToMarkdown(html) {
	...
	for (let i = 1; i <= 6; i++) {
		...
		markdown = markdown.replace(regex, (_match, content) => {
	...
	// 转换段落
	markdown = markdown.replace(/<p[^>]*>(.*?)<\/p>/gs, (_match, content) => {
		return `${content.trim()}\n\n`;
	});
...
function convertPostToMarkdown(post) {
	...
	const excerpt = post.post_excerpt
		? convertHTMLToMarkdown(post.post_excerpt)
		: "";
...
function main() {
	...
	for (const post of posts) {
		try {
			const { frontmatter, content, filename, originalSlug } =
